...
parfor parIdx=1:numWorkers
  ...
  while frameIdx < numFramesPerWorker
    ...
    for imgSizeIndex=1:length(imageSize)
       ...
       % Pass through channel (apply multipath fading first)
        rxWaveChan = multipathChannel5G(txWave, sr, Doppler, Fc);
        
        % Interference parameters
        interfPower = 10^(-5);  % Interference power
        t = (0:length(rxWaveChan)-1) / sr;  % Time vector based on received signal
        
        % Randomly choose between narrowband or broadband interference
        interfType = rand();  % Generate a random number between 0 and 1
        
        % Narrowband Interference (sinusoidal tone) or Broadband Interference (random noise)
        if interfType < 0.5  % 50% chance for narrowband interference
            % Narrowband interference (sinusoidal tone)
            narrowbandInterfSignal = sqrt(interfPower) * cos(2*pi*(Fc + 5e6)*t)';  % Interferer at Fc + 5 MHz
            interfSignal = narrowbandInterfSignal;
        else  % 50% chance for broadband interference
            % Broadband interference (random noise)
            broadbandInterfSignal = sqrt(interfPower) * randn(length(t), 1);  % Random noise interference
            interfSignal = broadbandInterfSignal;
        end
        
        % Pass signal through fading channel first
        % Randomly choose between Rayleigh and Rician fading
        useRician = rand() > 0.5; % 50% chance to use Rician
        
        if useRician
            % Apply Rician fading
            fadingChan = comm.RicianChannel( ...
                'SampleRate', sr, ...
                'KFactor', 5, ... % Adjust K-factor (higher = stronger LOS)
                'MaximumDopplerShift', Doppler);
        else
            % Apply Rayleigh fading
            fadingChan = comm.RayleighChannel( ...
                'SampleRate', sr, ...
                'MaximumDopplerShift', Doppler);
        end
        
        % Pass the combined signal through the selected fading channel
        rxWaveWithFading = fadingChan(rxWaveChan);
        
        % Add interference to the faded signal
        rxWaveWithInterference = rxWaveWithFading + interfSignal;
        
        % Add noise to the signal with interference
        rxWave = awgn(rxWaveWithInterference, SNRdB, 'measured');
...
[rxSpectrogram,P] = helperSpecSenseSpectrogramImage(rxWave, Nfft, sr, imageSize);
